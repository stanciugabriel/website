...
To blink a led using `embedded-hal`, take a look at [Configuring GPIO Output in Embassy](../lab/01#configure-gpio-output), from Lab 2. Embassy uses `embedded-hal`, so the functions for setting pin values to HIGH or LOW would be the same.
